...
class Eventmanagement extends BaseController
{
	...
	public function index()
	{
		...
		$baseUrl 							= 	base_url() . 'hhjsitemgmt/eventmanagement/index';
		...
		$this->layouts->admin_view('event/index', array(), $data);
	}	// END OF FUNCTION
	...
	public function addeditdata($editId = '')
	{
		...
		if ($editId) :
			...
			if (is_array($data['SELECTED_JAZZ_TYPES']))
     ...
     $postData = $this->request->getPost();
		...
		$this->layouts->set_title('Manage Event');
		$this->layouts->admin_view('event/addeditdata', array(), $data);
	}	// END OF FUNCTION	
	...
	{
		if ($this->request->isAJAX()) { // Ensure the request is AJAX
			$locationId = $this->request->getGet('LocationId'); // Securely get input

			if (empty($locationId)) {
				return $this->response->setJSON(['status' => 'error', 'message' => 'Location ID is required.']);
			}

			$db = \Config\Database::connect(); // Get DB connection
			$builder = $db->table('event_location_tbl');

			$builder->select('
				event_location_tbl.*, 
				event_location_type.name AS event_location_type_name
			');
			$builder->join('event_location_type', 'event_location_type.id = event_location_tbl.event_location_type_id', 'left');
			$builder->where('event_location_tbl.id', $locationId);

			$query = $builder->get();
			$dataQuery = $query->getRowArray(); // Get single row as an array
			...
			if ($dataQuery) {
				return $this->response->setJSON([
					...
					'data' => [
						...
						'event_location_type_name' => $dataQuery['event_location_type_name'] ?? '',
						'artist_id' => $dataQuery['artist_id'] ?? '',
					]
				]);
			} else {
				return $this->response->setJSON(['status' => 'error', 'message' => 'Location not found.']);
			}
		} else {
