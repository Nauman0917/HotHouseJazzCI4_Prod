...
class Previous_issue extends BaseController {
...
$this->lang->setLocale('front');
	...
	public function index()
	{
		$data = array();
	/********************************************Banner Section******************************/
	$where['where'] 		=	['page_name' => 'Previous Issues Page',  'is_active' =>'1'];//"status = 'A'";
...
/********************************************Year Section******************************/
       $where1['where'] 		=	"is_active = '1'";//"status = 'A'";
		$tbl1 					=	'previous_issues_tbl as ftable';
		$shortField12 			=	'id DESC';

		$shortField123 			=	'type_name ASC';
		 $data['previous_issues_tbl'] 		= 	$this->common_model->getData('multiple', $tbl1,$where1,$shortField12,6,0);
	 ...
	 /********************************************Seo Section******************************/
$where5['where'] 		=	['page_name' => 'Previous Issues Page',  'is_active' =>'1'];
...
if($this->request->getPost('Savesubsc')):
	...
	$this->session->setFlashdata('alert_error','Please Enter All Details');
		...
		$this->layouts->set_keyword($data['seo_section']['keywords']);
		 $this->layouts->front_view('previous_issue',array(),$data);
	}
}
