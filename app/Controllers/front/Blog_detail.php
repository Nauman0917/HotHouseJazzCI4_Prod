...
class Blog_detail extends BaseController {
...
$this->lang->setLocale('front');
	...
	$where1['where'] 		=	$this->common_model->getDataByParticularField('blog_tbl','slug',$editId);
	$tbl1 					=	'blog_tbl as ftable';
  	$data['blog'] 	= 	$this->common_model->getData('multiple',$tbl1 ,$where1);
	  ...
	  if ($this->request->getPost('Savesubsc')) {
		...
		$param = [
			...
			if(empty($subscribe)){
				...
				//$this->layouts->front_view('blog_detail',array(),$data);
				// return redirect()->to('/');
			}
