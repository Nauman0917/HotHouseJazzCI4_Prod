...
$this->lang->setLocale('front');
	...
	$data = array();
	/********************************************Banner Section******************************/
	$where['where'] 		=	['page_name' => 'Blog Page',  'is_active' =>'1'];//"status = 'A'";
	...
	$tbl1 					=	'blog_tbl as ftable';
	$shortField_ 			=	'position ASC';

	//$shortField11 			=	'type_name ASC';

	$data['blog_tbl'] 		= 	$this->common_model->getData('multiple', $tbl1,$where1,$shortField_,6,0);
//echo $this->db->last_query();exit;
	 ...
	 $config["total_rowsw"] = $this->pagination_model->get_desc();
	 $config["per_page"] = 6;
	 ...
	 $data['authors'] = $this->pagination_model->get_authors($config["per_page"], $page);
...
if($this->request->getPost('Savesubsc')):
	...
	$this->session->setFlashdata('alert_error','Please Enter All Details');
		...
		$this->layouts->set_keyword($data['seo_section']['keywords']);
		 $this->layouts->front_view('bloglist',array(),$data);
	}
}
